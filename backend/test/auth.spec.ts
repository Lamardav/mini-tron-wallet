...
function makeAuthService() {
  ...
  const crypto = { encrypt: jest.fn().mockReturnValue('encrypted-private-key') };
  ...
  return {
    ...
    crypto,
    jwt,
    service: new AuthService(prisma as never, jwt, tron as never, crypto as never),
  };
}
...
describe('AuthService.register', () => {
  it('creates a wallet, hashes the password and encrypts the private key', async () => {
    ...
    expect(created.wallet.create.encryptedPrivateKey).toBe('encrypted-private-key');
  ...
  it('never exposes the private key to the caller', async () => {
    ...
    prisma.user.create.mockResolvedValue({
      ...
      email: credentials.email,
      passwordHash: 'hash',
      wallet: { address: 'TVTqhMdd6mSQ3VanpwZvAn3H3zsUG8v7Ay', encryptedPrivateKey: 'encrypted' },
    });
  ...
  it('rejects an email that is already registered', async () => {
    ...
    await expect(service.register(credentials)).rejects.toThrow(ConflictException);
  });
});
